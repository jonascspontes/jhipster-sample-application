...
/**
 * REST controller for managing {@link io.github.jhipster.application.domain.Plataforma}.
 */
@RestController
@RequestMapping("/api")
public class PlataformaResource {
    ...
    public List<Plataforma> getAllPlataformas(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Plataformas");
        return plataformaRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Plataforma> getPlataforma(@PathVariable Long id) {
        ...
        Optional<Plataforma> plataforma = plataformaRepository.findOneWithEagerRelationships(id);
